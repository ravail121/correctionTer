...
$bulk_messages['post']     = array(
	...
	/* translators: %s: Number of posts. */
	'trashed'   => _n( '%s post moved to the Trash.', '%s posts moved to them Trash.', $bulk_counts['trashed'] ),
	/* translators: %s: Number of posts. */
...
<?php
// DEBUG: Show SQL query and fetched posts for admin Posts screen

global $wp_query, $wpdb;

echo '<h2>DEBUG: SQL Query</h2><pre>';
print_r( $wp_query->request );
echo '</pre>';

echo '<h2>DEBUG: Fetched Posts</h2><pre>';
print_r( $wp_query->posts );
echo '</pre>';

echo '<h2>DEBUG: Raw WPDB Queries (optional)</h2><pre>';
