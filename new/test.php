...
foreach ($companies as $symbol => $name) {
    try {
        // Fetch 'todaysChangePerc' from snapshot API
        ...
        // Update today's change percentage in the database
        $updateStmt = $con->prepare("UPDATE companies SET todaysChangePerc=? WHERE symbl=?");
        if (!$updateStmt) {
            throw new Exception("Prepare statement failed: " . $con->error);
        }
        $updateStmt->bind_param("ss", $todaysChangePerc, $symbol);
        if (!$updateStmt->execute()) {
            throw new Exception("Execute failed: " . $updateStmt->error);
        }

        echo "Today's change percentage for $name updated successfully.\n";
        $updateStmt->close();
    } catch (Exception $e) {
